...
def _write_progress(current: int, name: str, total: int) -> None:
    """把当前发送进度写入进度文件（供 GUI 实时展示），环境变量未配置时跳过。"""
    import os
    prog = os.getenv("DOUYIN_PROGRESS_FILE")
    if not prog:
        return
    try:
        Path(prog).write_text(
            json.dumps({"current": current, "name": name, "total": total}, ensure_ascii=False),
            encoding="utf-8",
        )
    except Exception:
        pass
        ...
        async with open_douyin(settings) as session:
            ...
            if fatal_error is None:
                ...
                for index, target in enumerate(task.targets):
                    ...
                    try:
                        _write_progress(index + 1, target.name, len(task.targets))
